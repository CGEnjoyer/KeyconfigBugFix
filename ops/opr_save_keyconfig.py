...
class KBF_OT_SaveKeyconfig(bpy.types.Operator):
    bl_idname = "kbf.save_keyconfig"
    bl_label = "Save Keyconfig"
    bl_description = "Save active keyconfig without addon keys"

    keyconfig_restore: bpy.props.BoolProperty(name="Restore Keyconfig (All Keymaps)", default=False)

    def execute(self, context):
        kcs = bpy.context.window_manager.keyconfigs
        kc_addon = kcs.addon
        kc_addon_restore = kcs['addon_restore']
        ...
        if kcs.active.name in ['Blender', 'Blender_27x', 'Industry_Compatible']:
            bpy.ops.wm.keyconfig_preset_add(name=kcs.active.name + "_Redefined")
        else:
            bpy.ops.wm.keyconfig_preset_add(name=kcs.active.name)

        # copy keymaps from tmp to addon
        #keymap_items_copy(kc_tmp, kca)
        #kcs.remove(kc_tmp)

        if self.keyconfig_restore: restore_keyconfig()

        return {'FINISHED'}


class KBF_OT_SaveCurrentKeyconfigAs(bpy.types.Operator):
    bl_idname = "kbf.save_keyconfig_as"
    ...
    def execute(self, context):
        ...
        if self.keymaps_restore: restore_keyconfig()
...
classes = (KBF_OT_SaveKeyconfig, KBF_OT_SaveCurrentKeyconfigAs)
