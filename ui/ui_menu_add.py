import bpy
from ..utils.utl_directories_manager import get_preferences
...
def menu_add_draw_USERPREF_MT_save_load(self, context):
    ...
    if prefs.menu_save_keyconfig:
        self.layout.operator("kbf.save_keyconfig", text="Save Keyconfig").keyconfig_restore = False
    if prefs.menu_save_keyconfig_as:
        self.layout.operator("kbf.save_keyconfig_as", text="Save Keyconfig As...")
    if prefs.menu_save_and_restore:
        self.layout.operator("kbf.save_keyconfig", text="Save & Restore Keyconfig").keyconfig_restore = True

    if prefs.menu_restore_keyconfig or prefs.menu_remove_addon_keymap_items:
        self.layout.separator()

    if prefs.menu_restore_keyconfig:
        self.layout.operator("kbf.restore_keyconfig", text="Restore All Keymaps")
